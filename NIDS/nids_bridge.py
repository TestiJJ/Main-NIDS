...
# Firebase Web API Key for Authenticating Users via Email/Password
FIREBASE_WEB_API_KEY = "AIzaSyCVS9na3K2hE9yyWWJulcBKHPXFkiMRExk"

# SMTP Email Notification Credentials (Sender System - Your Master Key)
SMTP_SERVER = "smtp.gmail.com"
SMTP_PORT = 587
SENDER_EMAIL = "testimonyjokotoye65@gmail.com"
SENDER_PASSWORD = "ovqscidivcalbowg"

# Alert Throttling & Suppression Window (Seconds)
...
try:
    ...
    print("✅ Connected to Firebase Cloud Engine")
...
def authenticate_user():
    """Authenticates the user against Firebase Auth and sets alert delivery email."""
    if "--logout" in sys.argv and os.path.exists(CONFIG_FILE):
        os.remove(CONFIG_FILE)
        print("🚪 Logged out active sensor session.")

    # 1. Check if saved configuration exists
    if os.path.exists(CONFIG_FILE):
        try:
            with open(CONFIG_FILE, 'r') as f:
                config = json.load(f)
                if config.get("uid") and config.get("email") and config.get("alert_email"):
                    print(f"🔑 Active Sensor Owner: {config['email']} (UID: {config['uid']})")
                    print(f"📧 Threat Notifications Routed to: {config['alert_email']}")
                    return config["uid"], config["email"], config["alert_email"]
        except Exception:
            pass

    # 2. Prompt for credentials if no config exists
    print("\n" + "="*50)
    ...
    # Prompt user for destination alert email (defaults to login email if left blank)
    print("\n📩 NOTIFICATION SETUP:")
    alert_email = input(f"Enter Email for Instant Threat Alerts [Press ENTER for {user_email}]: ").strip()
    if not alert_email:
        alert_email = user_email

    # Save to config file for continuous background execution
    ...
    return uid, user_email, alert_email


def send_critical_email(recipient_email, payload):
    """Dispatches formatted HTML critical alert email with built-in deduplication throttling."""
    ...
    # Create unique deduplication key for signature + attacker IP combination
...
def get_ip_location(ip_address):
    """Fetches geographical data for a given IP."""
...
def run_bridge():
    ...
    sensor_uid, owner_email, alert_email = authenticate_user()
    
    print(f"🛡️ NIDS LIVE: Routing logs to user path [/users/{sensor_uid}/network_alerts]...")
    user_ref = db.reference(f'users/{sensor_uid}/network_alerts')
        ...
        while True:
            ...
            try:
                ...
                if data.get("event_type") == "alert":
                    ...
                    # 1. Automatically push every alert to owner's database tree for records
                    user_ref.push(payload)
                    print(f"🚀 Cloud Alert Pushed to Dashboard for [{owner_email}]")
                    
                    # 2. Automatically dispatch critical email (throttled/deduplicated via cooldown window)
